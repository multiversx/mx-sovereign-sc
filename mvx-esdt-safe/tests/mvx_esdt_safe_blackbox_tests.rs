...
use common_test_setup::constants::{
    ...
    SOV_TOKEN, TESTING_SC_ADDRESS, TESTING_SC_ENDPOINT, TRUSTED_TOKEN, UNPAUSE_CONTRACT_LOG,
...
fn test_register_token_invalid_type() {
    ...
    let expected_logs = vec![
        log!(REGISTER_TOKEN_ENDPOINT, topics: [DEPOSIT_EVENT]),
        log!(REGISTER_TOKEN_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT], data: Some(INVALID_TYPE)),
    ];
...
fn test_register_token_not_enough_egld() {
    ...
    let expected_logs = vec![
        log!(REGISTER_TOKEN_ENDPOINT, topics: [DEPOSIT_EVENT]),
        log!(REGISTER_TOKEN_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT], data: Some(NOT_ENOUGH_EGLD_FOR_REGISTER)),
    ];
...
fn test_register_token_fungible_token() {
    ...
    let expected_logs = vec![log!(REGISTER_TOKEN_EVENT, topics: [EXECUTED_BRIDGE_OP_EVENT])];
    state.register_token(register_token_args, hash_of_hashes, None, expected_logs);
...
fn test_register_token_nonfungible_token() {
    ...
    let expected_logs = vec![
        log!(REGISTER_TOKEN_ENDPOINT, topics: [DEPOSIT_EVENT]),
        log!(REGISTER_TOKEN_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT], data: Some(INVALID_PREFIX_FOR_REGISTER)),
    ];
    state.register_token(register_token_args, hash_of_hashes, None, expected_logs);
...
fn test_execute_operation_no_chain_config_registered() {
    ...
    let expected_logs = vec![
        log!(EXECUTE_BRIDGE_OPS_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT], data: Some(CALLER_NOT_FROM_CURRENT_SOVEREIGN)),
    ];
...
fn test_deposit_execute_switch_mechanism() {
    ...
    let expected_logs = vec![log!(EXECUTE_OPERATION_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT])];
    state.execute_operation(&hash_of_hashes_one, &operation_one, expected_logs);
    ...
    let expected_logs = vec![log!(EXECUTE_OPERATION_ENDPOINT, topics: [EXECUTED_BRIDGE_OP_EVENT])];
    state.execute_operation(&hash_of_hashes_two, &operation_two, expected_logs);
