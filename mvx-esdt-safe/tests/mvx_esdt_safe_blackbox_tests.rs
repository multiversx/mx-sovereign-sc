use common_test_setup::constants::{
    ...
    TESTING_SC_ADDRESS, TESTING_SC_ENDPOINT, UNPAUSE_CONTRACT_LOG, USER_ADDRESS, WEGLD_IDENTIFIER,
};
...
fn test_register_token_invalid_type() {
    let mut state = MvxEsdtSafeTestState::new();
    state.deploy_and_complete_setup_phase(None);
...
fn test_deposit_transfer_data_only_with_fee_nothing_to_transfer() {
    ...
    state.deploy_and_complete_setup_phase(Some(fee));
...
fn test_set_token_burn_mechanism_no_roles() {
    ...
    state.set_token_burn_mechanism(
        WEGLD_IDENTIFIER.as_str(),
        Some(MINT_AND_BURN_ROLES_NOT_FOUND),
    );
}
