...
async fn test_switch_mechanism_with_deposit() {
    ...
    let trusted_token_id = chain_interactor.state().get_trusted_token().token_id;
    let trusted_token = chain_interactor.state().get_trusted_token();

    let trusted_token_info =
        chain_interactor.clone_token_with_amount(trusted_token, BigUint::from(ONE_HUNDRED_TOKENS));
