...
use common_test_setup::constants::{
    ...
    ONE_HUNDRED_TOKENS, PER_GAS, PER_TRANSFER, SC_CALL_EVENT, SHARD_0, SHARD_1,
...
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn test_execute_operation_transfer_data_only_sync_call_in_endpoint() {
    let mut chain_interactor = MvxEsdtSafeInteract::new(Config::chain_simulator_config()).await;
    ...
    let gas_limit = 90_000_000u64;
    let function = ManagedBuffer::<StaticApi>::from("view_storage");
...
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn test_execute_operation_transfer_data_only_async_call_in_endpoint() {
    let mut chain_interactor = MvxEsdtSafeInteract::new(Config::chain_simulator_config()).await;
    ...
    let gas_limit = 90_000_000u64;
    let function = ManagedBuffer::<StaticApi>::from("view_storage_async");
    let args = ManagedVec::<StaticApi, ManagedBuffer<StaticApi>>::from(vec![
