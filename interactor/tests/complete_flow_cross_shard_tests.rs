...
use common_interactor::interactor_state::TokenProperties;
use common_interactor::{
    common_sovereign_interactor::CommonInteractorTrait, interactor_state::TokenBalance,
};
use common_test_setup::constants::{
    DEPLOY_COST, ONE_HUNDRED_TOKENS, ONE_THOUSAND_TOKENS, OPERATION_HASH_STATUS_STORAGE_KEY,
    SHARD_0, SHARD_2, SOVEREIGN_RECEIVER_ADDRESS, TESTING_SC_ENDPOINT,
};
...
async fn test_complete_deposit_flow_no_fee_different_shard() {
    ...
    let expected_changed_balances_wallet = vec![
    ...
    chain_interactor
        .check_address_balance(
            ...
            expected_changed_balances_wallet,
        )
        .await;

    let expected_changed_balance_contract = vec![
        chain_interactor.custom_amount_tokens(
            chain_interactor.state.get_first_token_id_string(),
            ONE_HUNDRED_TOKENS,
        ),
    ...
    chain_interactor
        .check_address_balance(
            &chain_interactor
                ...
                .clone(),
            expected_changed_balance_contract,
        )
        .await;
    chain_interactor
...
async fn test_execute_operation_success_no_fee_different_shard_transfer_nft() {
    ...
    let expected_changed_balance_wallet =
        vec![chain_interactor.zero_tokens(nft_token.token_id.clone())];
...
async fn test_execute_operation_success_no_fee_different_shard_transfer_dynamic_nft() {
    ...
    let expected_changed_balance_wallet =
        vec![chain_interactor.zero_tokens(dynamic_nft.token_id.clone())];
...
///
/// ### ACTION
/// Call 'execute_operation()' to transfer a dynamic SFT with no fee
///
/// ### EXPECTED
/// The operation is executed and the tokens are received in the expected wallet
#[tokio::test]
...
async fn test_execute_operation_success_no_fee_different_shard_transfer_dynamic_sft() {
    ...
    let dynamic_sft: TokenProperties = chain_interactor.state.get_dynamic_sft_token_id();
    let payment = OperationEsdtPayment::new(
        TokenIdentifier::from_esdt_bytes(dynamic_sft.clone().token_id),
        dynamic_sft.nonce,
        token_data,
    );
    ...
    let mut payment_vec = PaymentsVec::new();
    payment_vec.push(EsdtTokenPayment {
        token_identifier: TokenIdentifier::from_esdt_bytes(dynamic_sft.token_id.clone()),
        token_nonce: dynamic_sft.nonce,
        amount: BigUint::from(ONE_HUNDRED_TOKENS),
    });
...
///
/// ### ACTION
/// Call 'execute_operation()' to transfer a dynamic META-ESDT with no fee
///
/// ### EXPECTED
/// The operation is executed and the tokens are received in the expected wallet
#[tokio::test]
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn test_execute_operation_success_no_fee_different_shard_transfer_dynamic_meta_esdt() {
    let mut chain_interactor = SovereignForgeInteract::new(Config::chain_simulator_config()).await;
    let shard = SHARD_0;
    ...
    let dynamic_meta_esdt: TokenProperties =
        chain_interactor.state.get_dynamic_meta_esdt_token_id();
    let payment = OperationEsdtPayment::new(
        TokenIdentifier::from_esdt_bytes(dynamic_meta_esdt.clone().token_id),
        dynamic_meta_esdt.nonce,
        token_data,
    );
    ...
    let mut payment_vec = PaymentsVec::new();
    payment_vec.push(EsdtTokenPayment {
        token_identifier: TokenIdentifier::from_esdt_bytes(dynamic_meta_esdt.token_id.clone()),
        token_nonce: dynamic_meta_esdt.nonce,
        amount: BigUint::from(ONE_HUNDRED_TOKENS),
    });
    ...
    let expected_changed_balance_wallet = vec![chain_interactor.custom_amount_tokens(
        dynamic_meta_esdt.token_id.clone(),
        ONE_THOUSAND_TOKENS - ONE_HUNDRED_TOKENS,
    )];
...
async fn test_execute_operation_success_with_fee_different_shard_transfer_dynamic_nft() {
    ...
    let expected_changed_balance_wallet = vec![chain_interactor.zero_tokens(
        chain_interactor
            .state
    ...
    let expected_changed_balance = vec![chain_interactor.custom_amount_tokens(
        chain_interactor.state.get_fee_token_id_string(),
        BigUint::from(ONE_THOUSAND_TOKENS) - fee_amount.clone(),
    )];
    chain_interactor
        .check_address_balance(&Bech32Address::from(user_address), expected_changed_balance)
        .await;

    chain_interactor
        .check_mvx_esdt_safe_balance_is_empty(shard)
        .await;
    let expected_token_fee_market = vec![TokenBalance {
        token_id: chain_interactor.state.get_fee_token_id().to_string(),
        amount: fee_amount,
    }];
    chain_interactor
        .check_address_balance(
...
async fn test_complete_flow_execute_operation_with_transfer_data_success_nft_no_fee() {
    ...
    let expected_tokens_wallet =
        vec![chain_interactor.zero_tokens(chain_interactor.state.get_nft_token_id_string())];
    ...
    let expected_testing_sc_balance = vec![TokenBalance {
        token_id: chain_interactor.state.get_nft_token_id_string(),
        amount: BigUint::from(1u64),
    }];
    chain_interactor
        .check_address_balance(
...
async fn test_complete_flow_execute_operation_with_transfer_data_success_sft_no_fee() {
    ...
    let expected_changed_balances = vec![chain_interactor.custom_amount_tokens(
    ...
    let expected_changed_testing_sc_balance = vec![TokenBalance {
        token_id: chain_interactor.state.get_sft_token_id_string(),
        amount: BigUint::from(ONE_HUNDRED_TOKENS),
    }];
    chain_interactor
        .check_address_balance(
            &chain_interactor.state.current_testing_sc_address().clone(),
            expected_changed_testing_sc_balance,
        )
        .await;
}
...
async fn test_complete_flow_execute_operation_with_transfer_data_success_meta_esdt_no_fee() {
    ...
    let expected_changed_testing_sc_balance = vec![TokenBalance {
        token_id: chain_interactor.state.get_meta_esdt_token_id_string(),
        amount: BigUint::from(ONE_HUNDRED_TOKENS),
    }];
    chain_interactor
        .check_address_balance(
...
async fn test_complete_flow_execute_operation_with_transfer_data_success_dynamic_nft_no_fee() {
    ...
    let expected_changed_testing_sc_balance = vec![TokenBalance {
        token_id: chain_interactor.state.get_dynamic_nft_token_id_string(),
        amount: BigUint::from(1u64),
    }];
    chain_interactor
        .check_address_balance(
...
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn test_complete_flow_execute_operation_with_transfer_data_success_dynamic_sft_no_fee() {
    let mut chain_interactor = SovereignForgeInteract::new(Config::chain_simulator_config()).await;
    let shard = SHARD_0;
    ...
    let payment = OperationEsdtPayment::new(
        TokenIdentifier::from_esdt_bytes(
            chain_interactor.state.get_dynamic_sft_token_id().token_id,
        ),
        chain_interactor.state.get_dynamic_sft_token_id().nonce,
        token_data,
    );
    let mut payment_vec = PaymentsVec::new();
    payment_vec.push(EsdtTokenPayment {
        ...
        token_nonce: chain_interactor.state.get_dynamic_sft_token_id().nonce,
        amount: BigUint::from(ONE_HUNDRED_TOKENS),
    });
    ...
    chain_interactor
        .deposit_in_mvx_esdt_safe(
            ...
            payment_vec,
            None,
            Some(&chain_interactor.state.get_dynamic_sft_token_id_string()),
        )
        .await;
    ...
    chain_interactor
        .execute_operations_in_mvx_esdt_safe(
            ...
            Some(
                chain_interactor
                    .state
                    .get_dynamic_sft_token_id_string()
                    .as_str(),
            ),
            None,
        )
    ...
    let expected_changed_balance_wallet = vec![chain_interactor.custom_amount_tokens(
        chain_interactor.state.get_dynamic_sft_token_id_string(),
        ONE_THOUSAND_TOKENS - ONE_HUNDRED_TOKENS,
    )];
    chain_interactor
        .check_address_balance(
    ...
    let expected_changed_testing_sc_balance = vec![chain_interactor.custom_amount_tokens(
        chain_interactor.state.get_dynamic_sft_token_id_string(),
        ONE_HUNDRED_TOKENS,
    )];
    chain_interactor
...
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn test_complete_flow_execute_operation_with_transfer_data_success_dynamic_meta_esdt_no_fee()
{
    let mut chain_interactor = SovereignForgeInteract::new(Config::chain_simulator_config()).await;
    let shard = SHARD_0;
    ...
    let payment = OperationEsdtPayment::new(
        TokenIdentifier::from_esdt_bytes(
            chain_interactor
                .state
                .get_dynamic_meta_esdt_token_id()
                .token_id,
        ),
        chain_interactor
            .state
    ...
    chain_interactor
        .deposit_in_mvx_esdt_safe(
            ...
            Some(
                &chain_interactor
                    .state
                    .get_dynamic_meta_esdt_token_id_string(),
            ),
        )
        .await;
...
async fn test_complete_flow_execute_operation_with_transfer_data_success_dynamic_nft_with_fee() {
    ...
    let expected_changed_fee_market_balance = vec![TokenBalance {
    ...
    chain_interactor
        .check_address_balance(
            &chain_interactor.state.get_fee_market_address(shard).clone(),
            expected_changed_fee_market_balance,
        )
        .await;
