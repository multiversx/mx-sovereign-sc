...
async fn main() {
    ...
    match cmd.as_str() {
        ...
        "deposit" => interact.deposit(OptionalTransferData::None, Option::None).await,
...
impl ContractInteract {
    ...
    async fn deploy(&mut self, is_sovereign_chain: bool) {
        let opt_wegld_identifier = Option::Some(TokenIdentifier::from_esdt_bytes(WHITELIST_TOKEN_ID));
        let opt_sov_token_prefix = Option::Some(ManagedBuffer::new_from_bytes(&b"sov"[..]));
        let token_handler_address = managed_address!(self.state.token_handler_address.clone().unwrap().as_address());
        ...
        let new_address = self
            ...
            .init(is_sovereign_chain, token_handler_address, opt_wegld_identifier, opt_sov_token_prefix)
    ...
    async fn deploy_header_verifier(&mut self) {
        ...
        let new_address_bech32 = bech32::encode(&new_address);
        self.state
            .set_header_verifier_address(Bech32Address::from_bech32_string(new_address_bech32.clone()));
    ...
    async fn deploy_fee_market(&mut self) {
        ...
        let fee = FeeStruct {
            ...
            fee_type: FeeType::Fixed { token: TokenIdentifier::from_esdt_bytes(TOKEN_ID), per_transfer: BigUint::from(10u64), per_gas: BigUint::from(0u64) },
        };

        let price_aggregator_address = managed_address!(self.state.price_aggregator_address.clone().unwrap().as_address());
        ...
        let new_address = self
            ...
            .init(self.state.current_address(), price_aggregator_address, Option::Some(fee))
        ...
        let new_address_bech32 = bech32::encode(&new_address);
        self.state
            .set_fee_market_address(Bech32Address::from_bech32_string(new_address_bech32.clone()));
    ...
    async fn deploy_token_handler(&mut self) {
        ...
        let new_address_bech32 = bech32::encode(&new_address);
        self.state
            .set_token_handler_address(Bech32Address::from_bech32_string(new_address_bech32.clone()));
    ...
    async fn deploy_price_aggregator(&mut self) {
        ...
        let new_address = self
            ...
            .init(TokenIdentifier::from_esdt_bytes(TOKEN_ID), BigUint::from(1u64), BigUint::from(1u64), 3u8, 3u8,  oracles)
        ...
        let new_address_bech32 = bech32::encode(&new_address);
        self.state
            .set_price_aggregator_address(Bech32Address::from_bech32_string(new_address_bech32.clone()));
        println!("new token_handler_address: {new_address_bech32}");
    }
    ...
    async fn deposit(&mut self, transfer_data: OptionalTransferData<StaticApi>, error_wanted: Option<ExpectError<'_>>) {
        let token_id = TOKEN_ID;
        let token_nonce = 0u64;
        let token_amount = BigUint::from(20u64);
        let to = &self.bob_address;
        let mut payments = PaymentsVec::new();
        payments.push(EsdtTokenPayment::new(TokenIdentifier::from(token_id), token_nonce, token_amount.clone()));
    ...
    async fn execute_operations(&mut self) {
        ...
        let operation = Operation::new(ManagedAddress::zero(), ManagedVec::new(), OperationData::new(0, ManagedAddress::zero(),Option::None));
    ...
    async fn add_tokens_to_whitelist(&mut self, token_id: &[u8]) {
        ...
        match token_id {
            WHITELIST_TOKEN_ID => {
                tokens = MultiValueVec::from(vec![TokenIdentifier::from_esdt_bytes(WHITELIST_TOKEN_ID)]);
            }
            TOKEN_ID => {
